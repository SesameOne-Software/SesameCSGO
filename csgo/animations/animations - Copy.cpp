...
bool animations::setup_bones ( player_t* target, matrix3x4_t* mat, int mask, vec3_t rotation, vec3_t origin, float time ) {
	...
	*reinterpret_cast< int* >( uintptr_t ( target ) + N ( 0xf0 ) ) |= N ( 8 );
	...
	const auto ret = target->setup_bones ( *( std::array< matrix3x4_t, 128 >* ) mat, 128, mask, time );
...
int animations::store_local ( ) {
	...
	if ( g::local->simtime ( ) != last_local_update ) {
		local_data::abs = animstate->m_abs_yaw;
...
int animations::restore_local ( ) {
	...
	/* storing shit for animations next time */
	...
	if ( g::send_packet && updated_tick != csgo::i::globals->m_tickcount ) {
		...
		if ( local_data::was_on_ground ) {
			...
			local_data::hit_ground_time = g::local->simtime ( );
		}

		last_update_time = g::local->simtime ( );
		g::hold_aim = false;
	}
	...
	animstate->m_feet_yaw_rate = 0.0f;
	animstate->m_unk_feet_speed_ratio = 0.0f;
	...
	if ( !csgo::i::input->m_camera_in_thirdperson ) {
		animstate->m_duck_amount = fd_key ? 0.0f : g::local->crouch_amount ( );
		animstate->m_unk_feet_speed_ratio = g::local->crouch_speed ( );
	}
