...
//precompiler instructions -> replace the _(string) with a _(_'d_string) so that
...
const wchar_t* security::internal::get_string(int index) {
	...
	switch (index) {
	case 0: value = _("Qt5QWindowIcon"); break;
...
int security::internal::memory::remote_debugger_present() {
	...
	h_process = LI_FN( GetCurrentProcess )();
	//check if a remote debugger is present
	LI_FN( CheckRemoteDebuggerPresent )(h_process, &found);
...
int security::internal::memory::check_window_name() {
	...
	for (const wchar_t* name : names) {
		if (LI_FN( FindWindowW )(name, 0)) { return security::internal::debug_results::find_window; }
	}
...
int security::internal::memory::nt_set_information_thread() {
	...
	(_NtSetInformationThread)(LI_FN( GetCurrentThread )(), thread_hide_from_debugger, 0, 0, 0);
...
int security::internal::memory::debug_active_process() {
	...
	LI_FN( CreateMutexW )(NULL, FALSE, get_string(5));
	...
	LI_FN( GetModuleFileName )(NULL, sz_path, MAX_PATH);

	char cmdline[MAX_PATH + 1 + sizeof(int)];
	LI_FN( snprintf )(cmdline, sizeof(cmdline), _ ("%ws %d"), sz_path, pid);
...
int security::internal::memory::write_buffer() {
	...
	PVOID* addresses = static_cast<PVOID*>(LI_FN( VirtualAlloc )(NULL, 4096 * sizeof(PVOID), MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE));
	if (addresses == NULL) { 
		return security::internal::debug_results::write_buffer; }

	int* buffer = static_cast<int*>( LI_FN( VirtualAlloc )(NULL, 4096 * 4096, MEM_RESERVE | MEM_COMMIT | MEM_WRITE_WATCH, PAGE_READWRITE));
	if (buffer == NULL) {
	...
	addresses = static_cast<PVOID*>( LI_FN( VirtualAlloc )(NULL, 4096 * sizeof(PVOID), MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE));
	if (addresses == NULL) { return security::internal::debug_results::write_buffer; }

	buffer = static_cast<int*>( LI_FN( VirtualAlloc )(NULL, 4096 * 4096, MEM_RESERVE | MEM_COMMIT | MEM_WRITE_WATCH, PAGE_READWRITE));
	if (buffer == NULL) {
		LI_FN( VirtualFree )(addresses, 0, MEM_RELEASE);
		return security::internal::debug_results::write_buffer;
	}
	...
	if (LI_FN( GlobalGetAtomName )(INVALID_ATOM, (LPTSTR)buffer, 1) != FALSE || LI_FN( GetEnvironmentVariableW )(get_string(6), (LPWSTR)buffer, 4096 * 4096) != FALSE || GetBinaryTypeW(get_string(7), (LPDWORD)buffer) != FALSE
		|| LI_FN( HeapQueryInformation )(0, (HEAP_INFORMATION_CLASS)69, buffer, 4096, NULL) != FALSE || LI_FN( ReadProcessMemory )(INVALID_HANDLE_VALUE, (LPCVOID)0x69696969, buffer, 4096, NULL) != FALSE
		|| LI_FN( GetThreadContext )(INVALID_HANDLE_VALUE, (LPCONTEXT)buffer) != FALSE || LI_FN( GetWriteWatch )(0, &security::internal::memory::write_buffer, 0, NULL, NULL, (PULONG)buffer) == 0) {
		result = false;
		error = true;
	}
...
int security::internal::exceptions::debug_string() {
	SetLastError(0);
	OutputDebugStringA(_ ("anti-debugging test."));
...
int security::internal::virtualization::check_registry() {
	HKEY h_key = 0;
	if (RegOpenKeyExW(HKEY_LOCAL_MACHINE, _ (L"HARDWARE\\ACPI\\DSDT\\VBOX__"), 0, KEY_READ, &h_key) == ERROR_SUCCESS) { return security::internal::debug_results::check_registry; }

	return security::internal::debug_results::none;
}

int security::internal::virtualization::vm() {
	if (CreateFileW(_ (L"\\\\.\\VBoxMiniRdrDN"), GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, 0, OPEN_EXISTING, 0, 0) != INVALID_HANDLE_VALUE) { return security::internal::debug_results::vm; }

	if (LoadLibraryW(_ (L"VBoxHook.dll"))) { return security::internal::debug_results::vm; }

	HKEY h_key = 0;
	if ((ERROR_SUCCESS == RegOpenKeyExW(HKEY_LOCAL_MACHINE, _(L"SOFTWARE\\Oracle\\VirtualBox Guest Additions"), 0, KEY_READ, &h_key)) && h_key) { RegCloseKey(h_key); return security::internal::debug_results::vm; }

	h_key = 0;
	if (RegOpenKeyExW(HKEY_LOCAL_MACHINE, _(L"HARDWARE\\DESCRIPTION\\System"), 0, KEY_READ, &h_key) == ERROR_SUCCESS)
	{
		unsigned long type = 0;
		unsigned long size = 0x100;
		char* systembiosversion = (char*)LocalAlloc(LMEM_ZEROINIT, size + 10);
		if (ERROR_SUCCESS == RegQueryValueExW(h_key, _(L"SystemBiosVersion"), 0, &type, (unsigned char*)systembiosversion, &size))
		{
		...
		if (ERROR_SUCCESS == RegQueryValueExW(h_key, _(L"VideoBiosVersion"), 0, &type, (unsigned char*)videobiosversion, &size))
		{
	...
	HANDLE h = CreateFileW(_(L"\\\\.\\pipe\\VBoxTrayIPC"), GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, 0, OPEN_EXISTING, 0, 0);
	...
	if ((ERROR_SUCCESS == RegOpenKeyExW(HKEY_LOCAL_MACHINE, w_subkey, 0, KEY_READ, &h_key)) && h_key)
	{
						...
						if ((RegOpenKeyExW(h_key, w_key_new, 0, KEY_READ, &h_new_key) == ERROR_SUCCESS) && h_new_key)
						{
							...
							if (RegEnumKeyW(h_new_key, 0, w_key_newer, maxlen + 1) == ERROR_SUCCESS)
							{
								...
								if (RegOpenKeyExW(h_new_key, w_key_newer, 0, KEY_READ, &HKKK) == ERROR_SUCCESS)
								{
									...
									if (RegQueryValueExW(h_new_key, _(L"FriendlyName"), 0, 0, value_name, &size) == ERROR_SUCCESS) { to_lower(value_name); if (strstr((char*)value_name, _("vbox"))) { return security::internal::debug_results::vm; } }
									RegCloseKey(HKKK);
								}
							}
