...
*
*	!!!IMPORTANT!!!
*	DISABLE SAFESEH (/SAFESEH[:NO]) OR ELSE VM DETECTION VIA SEH WILL NOT WORK!
*
*/
...
__forceinline int security::internal::memory::nt_set_information_thread( ) {
	...
	LPVOID* check = nullptr;

	if ( SUCCEEDED( NtSetInformationThread( NtCurrentThread, thread_hide_from_debugger, &check, sizeof ULONG ) ) )
		return security::internal::debug_results::nt_set_information_thread;

	if ( SUCCEEDED( NtSetInformationThread( HANDLE( 0xFFFFF ), thread_hide_from_debugger, nullptr, 0 ) ) )
		return security::internal::debug_results::nt_set_information_thread;
...
__forceinline int security::internal::memory::debug_active_process( ) {
	...
	BOOL success = LI_FN( CreateProcessA )(
		nullptr,		//path (nullptr means use cmdline instead)
...
__forceinline security::internal::debug_results security::check_security( ) {
	...
	// unreliable + won't work well with the amount of data our hack is processing
