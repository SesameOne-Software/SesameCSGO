...
public:
	...
	NETVAR( uint32_t, team, "DT_BaseEntity->m_iTeamNum" );
	NETVAR( uint32_t, highlight_r, "DT_BaseAnimating->m_nHighlightColorR" );
	NETVAR( uint32_t, highlight_g, "DT_BaseAnimating->m_nHighlightColorG" );
	NETVAR( uint32_t, highlight_b, "DT_BaseAnimating->m_nHighlightColorB" );
	NETVAR( uint32_t, model_scale, "DT_BaseAnimating->m_flModelScale" );
	...
	void draw( ) {
		using drawmodel_fn = int( __thiscall* )( void*, int, uint8_t );
