...
public:
	c_bone_accessor( ) {
		m_animating = nullptr;
		m_bones = nullptr;
		m_readable = m_writeable = 0;
	}

	c_bone_accessor( matrix3x4_t* bones ) {
		m_animating = nullptr;
		m_bones = bones;
	}

	void init( const void* animating, matrix3x4_t* bones ) {
...
public:
	...
	NETVAR( bool, clientside_animation, "DT_BaseAnimating->m_bClientSideAnimation" );
	...
	OFFSET( animlayer_t*, overlays, 0x2980 );
	...
	float* poseparam( ) {
		return ( float* ) ( std::uintptr_t( this ) + 0x2774 );
	}
