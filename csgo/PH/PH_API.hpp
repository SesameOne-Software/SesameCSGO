...
#define PH_HEARTBEAT_INIT_TASK 40
	...
	*/
	...
	__forceinline static int on_fail ( ) {
		OBF_BEGIN;
	...
	*/
	...
	__forceinline static int send_heartbeat ( int task = PH_HEARTBEAT_TASK ) {
		OBF_BEGIN;
		AES aes ( 256 );
		...
		CASE ( task )
		WHEN ( N ( PH_HEARTBEAT_INIT_TASK ) ) DO {
		...
		IF ( !json_return_buffer ) {
			ph_heartbeat::on_fail ( );
			RETURN ( 1 );
		} ENDIF;

		cJSON* json_return = cJSON_Parse ( json_return_buffer );

		free ( ( void* ) json_return_buffer );
		...
		IF ( success_value->type == N ( cJSON_False ) ) {
			ph_heartbeat::on_fail ( );
			RETURN ( 1 );
		} ENDIF;
		...
		IF ( task == N ( PH_HEARTBEAT_INIT_TASK ) ) {
			ph_heartbeat::ph_heartbeat_user_mutex.lock ( );
	...
	*/
	...
	__forceinline static int initialize_heartbeat ( ph_heartbeat::heartbeat_info* in_info ) {
		OBF_BEGIN;
		heartbeat_token_hashed = in_info->heartbeat_token_hashed;

		IF ( !client::create_socket ( in_info->host, in_info->port ) )
			ph_heartbeat::on_fail ( ); ENDIF;

		ph_heartbeat::send_heartbeat ( N( PH_HEARTBEAT_INIT_TASK ) );

		LI_FN( VirtualFree )( in_info, N( 0 ), N( MEM_RELEASE ) ); // Releasing data so token hashed is no longer in memory
		OBF_END;
	}
}