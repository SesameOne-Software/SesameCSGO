...
namespace oxui {
	namespace binds {
		...
		static std::add_pointer_t< void ( const oxui::pos&, int, int, const color&, bool ) > circle = [ ] ( const oxui::pos& pos, int rad, int segments, const color& c, bool outline ) {
			render::circle ( pos.x, pos.y, rad, segments, D3DCOLOR_RGBA ( c.r, c.g, c.b, c.a ), 0, 0.0f, outline );
		};
		...
		static std::add_pointer_t< void( const pos&, const font&, const str&, const color&, bool ) > text = [ ] ( const pos& point, const font& font, const str& text, const color& c, bool shadow ) {
			if ( shadow && c.a == 255 )
				render::text( point.x + 1, point.y + 1, D3DCOLOR_RGBA( 0, 0, 0, c.a ), ( void* ) font, text );

			render::text( point.x, point.y, D3DCOLOR_RGBA( c.r, c.g, c.b, c.a ), ( void* ) font, text );
		};

		static std::add_pointer_t< void ( const oxui::rect&, int, int, const color&, bool ) > rounded_rect = [ ] ( const oxui::rect& r, int rad, int resolution, const color& c, bool outline ) {
			render::rounded_rect ( r.x, r.y, r.w, r.h, rad, resolution, D3DCOLOR_RGBA ( c.r, c.g, c.b, c.a ), outline );
		};
	}
}
