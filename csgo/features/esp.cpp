						...
						if ( ( int ) abs ( LODWORD ( v35 ) - (int)g::server_tick ) <= ( int ) ( float ) ( ( float ) ( 1.0 / cs::i::globals->m_ipt ) + 0.5 ) ) {
							...
							LABEL_99:
								if ( ( unsigned int ) ( v25 - 1 ) <= 124 && !( bool ) v39 ) {
									...
									if ( player && player->is_player ( ) ) {
										if ( g::local && player->dormant ( ) && !player->life_state ( ) && player->team ( ) != g::local->team ( ) ) {
...
void features::esp::render( ) {
	...
	for ( auto i = 1; i <= cs::i::globals->m_max_clients; i++ ) {
		...
		if ( esp_data[ e->idx( ) ].m_pl && std::fabsf( cs::i::globals->m_curtime - esp_data[ e->idx( ) ].m_last_seen ) < dormant_time ) {
			...
			const auto& anim_info = anims::anim_info [ e->idx ( ) ];
			if ( !anim_info.empty( ) && anim_info.front( ).m_resolved )
				draw_esp_widget ( esp_rect, visuals.zoom_color, esp_type_text, visuals.value_text, visuals.zoom_flag_placement, esp_data [ e->idx ( ) ].m_dormant, 0.0, 0.0, _ ( "Resolved" ) );
